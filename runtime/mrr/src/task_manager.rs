...
impl TaskManager {
    ...
    pub async fn create_task(&self, command: CreateTaskCommand) -> Result<Task> {
        let owner = self.principal_store.get(command.owner_principal_id).await?;
    ...
    pub async fn start_task(&self, task_id: Uuid, actor: PrincipalAttribution) -> Result<Task> {
        self.ensure_active_principal(actor.principal_id).await?;
        let mut tx = self.pool.begin().await?;
    ...
    async fn ensure_active_principal(&self, principal_id: Uuid) -> Result<()> {
        let principal = self.principal_store.get(principal_id).await?;
